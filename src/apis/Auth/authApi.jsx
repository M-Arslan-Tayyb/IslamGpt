...
export function useLogout() {
    ...
    const logout = async () => {
        try {
            // Call backend API to clear Redis cache if needed (optional)
            // await apiConnector('POST', CLEAR_CACHE);

            // Clear user data from localStorage
            localStorage.removeItem('islamgpt_token');
            localStorage.removeItem('islamgpt_user');

            // Clear user data from Redux store
            // dispatch(clearUser());
            // Optionally clear all queries from React Query cache
            queryClient.clear();
