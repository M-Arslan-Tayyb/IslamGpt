...
import { Home, MessageCircle, Book, Calendar, MapPin, Clock, Brain, Menu, CircleX } from 'lucide-react'
...
const Sidebar = ({ isOpen, setIsOpen }) => {
    ...
    return (
        <>
            {/* Desktop Sidebar */}
            <aside className="hidden md:flex bg-white h-screen fixed left-0 top-16 border-r border-gray-200 transition-all duration-300 flex-col"
                                    ...
                                    <li>
                                        {!isOpen ? (
                                            <Tooltip content={item.description}>
                                                <Link
                                                    to={item.path}
                                                    className={`flex items-center px-2 justify-center py-3 text-xs rounded-lg transition-colors ${location.pathname === item.path
                                            ...
                                            <Link
                                                to={item.path}
                                                className={`flex items-center px-4 py-3 text-xs rounded-lg transition-colors ${location.pathname === item.path
                                                    ? 'bg-[var(--text-bg)] text-[var(--primary-color)] font-semibold'
                                                    : 'text-[var(--text-gray)] hover:bg-[var(--text-bg-hover)]'
                                                    }`}
                                            >
                ...
                <nav className="flex justify-evenly items-center px-4 py-2">
                    {mobileMenuItems.map((item) => (
                        <Link
                            key={item.path}
                            to={item.path}
                            className={`flex flex-col items-center justify-center px-3 py-1 hover:bg-[var(--text-bg-hover)] transition-all duration-300 rounded-md ${location.pathname === item.path
